...
**WIP** (veravanged door Danny)
Steerable convolutions, particularly SO(3) convolutions, extend the convolutional neural network (CNN) framework to be equivariant to a broader range of transformations, such as rotations, reflections, and scalings, beyond the simple translation equivariance of standard convolutions. This makes them especially suitable for tasks involving complex transformations like 3D object recognition. Unlike regular group convolutions, which handle specific transformations using separate filters and thus require increased memory, steerable convolutions utilize a set of basis filters combined linearly to achieve the desired transformation. This results in a much more memory-efficient design, as fewer parameters are needed to encode continuous group transformations like those in SO(3). The mathematical foundation of steerable convolutions is rooted in group theory and irreducible representations, which decompose complex transformations into simpler, invariant components. Filters are often constructed using spherical harmonics or other special functions, enabling efficient and accurate handling of rotations. This allows steerable convolutions to maintain partial translation equivariance, harnessing the benefits of classical convolutions while extending their capabilities. The primary advantages include reduced memory footprint and improved performance in recognizing objects and patterns irrespective of their orientation, offering a flexible and mathematically principled approach to incorporate various symmetries into neural networks.

Group equivariant convolutional neural networks (GCNNs) further enhance classical convolutional networks by introducing new inductive biases, such as rotational symmetry, which constrain the network to produce representations that comply with the specified symmetries. For example, a rotation in the input should result in a corresponding rotation in the output. This symmetry results in a more efficient learning process since fewer kernels are required to capture the same representational capacity. The ESCNN (Enhanced Steerable CNN) library can be used to implement such networks, allowing several isometries like rotations and reflections in both 2D and 3D spaces. Instead of traditional convolutions, the ESCNN uses steerable convolutions, which handle equivariance more efficiently in terms of memory than methods like lifted convolutions. This is particularly important for infinite groups such as 3D rotations, where memory utilization is already significant due to the nature of the data. In ESCNN, equivariant convolutions are achieved through utilizing feature maps of E(n)-equivariant steerable CNNs called feature fields, which transform predictably under operations like rotations and reflections. These feature fields can be scalar (like in gray-scale images) or n-dimensional vector fields, each transforming differently under such operations. Users can define input and output feature spaces for convolutions and create mappings similar to traditional convolutions, but with the added capability of handling group symmetries, thus facilitating the adoption of steerable convolutions into deep learning models.
...
Jesse: Discussed motivations behind hyperparameter and design choices, including LoRA parameters and learning rates. Investigated literature on adapter modules, cleaned the repository, and merged cleaned inference code with train scripts. Created a model diagram and a demo notebook of the SO3 patchembedding block. Implemented the pipeline for our contribution and conducted the inference pipeline setup with Danny. Produced visualizations and analyses for "Reproduction of the Experiments," and assisted with run executions, figures, and tables for other sections.
